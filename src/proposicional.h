...
namespace logica {
    ...
    public:
        ...
        void eval(std::ostream& os = std::cout) {
            if (!root) return;
            auto vars = getVars();
            ...
            for (char c : vars) os << " " << c << " |";
            for (auto& [s, _] : parts) os << " " << std::setw(s.size()) << s << " |";
            os << " " << expr << "\n";
            ...
            for (int i = total - 1; i >= 0; --i) {
                ...
                for (auto& [s, n] : parts) {
                    ...
                    os << " " << std::setw(s.size()) << (b ? 'V' : 'F') << " |";
                }
                os << " " << std::setw(expr.size()) << (evaluate(root, ctx) ? 'V' : 'F') << "\n";
            }
        }
